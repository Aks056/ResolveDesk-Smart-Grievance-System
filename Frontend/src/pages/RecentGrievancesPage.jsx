...
import { 
  Search, Filter, ChevronRight, Building2, Clock, CheckCircle2, AlertCircle, FileText, User2
} from "lucide-react";
    ...
    <div className="min-h-screen bg-background pb-20">
      <div className="container max-w-7xl mx-auto p-6 md:p-10 space-y-8 animate-in fade-in duration-700">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground font-medium">Viewing real-time redressal activity across AKTU.</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="relative w-full md:max-w-md">
            ...
            <Input 
              ...
              className="pl-10 h-11 rounded-xl bg-card border-border/40 focus:ring-primary/20 transition-all font-medium shadow-sm"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
          <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 no-scrollbar">
            {['ALL', 'PENDING', 'IN_PROGRESS', 'RESOLVED', 'CLOSED_BY_USER'].map((status) => (
              <Button
                key={status}
              ...
              >
                {status === 'CLOSED_BY_USER' ? 'ARCHIVED' : status}
              </Button>
            ))}
          </div>
        </div>
        ...
        <div className="grid gap-4">
          {loading ? (
            <div className="py-20 flex flex-col items-center gap-4">
              ...
              <p className="text-sm font-black uppercase tracking-[0.2em] text-muted-foreground animate-pulse">Syncing Portal...</p>
            </div>
            ...
            filteredGrievances.map((g) => (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center p-6 gap-6">
                  {/* Avatar / Icon */}
                  <div className={`hidden md:flex h-14 w-14 items-center justify-center rounded-2xl shrink-0 transition-colors bg-muted/50 text-muted-foreground group-hover:bg-primary/10 group-hover:text-primary`}>
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-black font-mono text-primary/60 tracking-widest">#GRV-{String(g.id).padStart(4, '0')}</span>
            ...
            <Card className="py-20 border-dashed border-2 bg-transparent text-center space-y-4">
              ...
              <div className="space-y-1">
                <h3 className="font-bold text-xl">No Grievances in Gallery</h3>
                <p className="text-muted-foreground text-sm max-w-[300px] mx-auto">
                  The system hasn't recorded any public grievances matching your criteria yet.
                </p>
              </div>
            </Card>
          )}
